...
def pixel_accuracy(output, target):
    ...
    with torch.no_grad():
        pred = torch.argmax(output, dim=1)  # Assuming your output is a softmax or has not been argmaxed yet
        correct = pred.eq(target).sum().item()
        total = target.nelement()  # Number of elements in the tensor
        return correct / total

def compute_iou(predicted, labels, num_classes):
    """
    Compute the Intersection over Union (IoU) for each class.

    Parameters:
    - predicted: a tensor of shape (N, H, W) with the predicted class IDs for each pixel.
    - labels: a tensor of shape (N, H, W) with the ground truth class IDs for each pixel.
    - num_classes: the total number of classes.

    Returns:
    - ious: a list of IoU values for each class.
    """
    ious = []
    predicted = predicted.squeeze(1)  # Assuming predicted is of shape (N, 1, H, W)
    for class_id in range(num_classes):
        true_positive = (predicted == class_id) & (labels == class_id)
        false_positive = (predicted == class_id) & (labels != class_id)
        false_negative = (predicted != class_id) & (labels == class_id)
        intersection = true_positive.sum().item()
        union = true_positive.sum().item() + false_positive.sum().item() + false_negative.sum().item()
        if union == 0:
